...
def MLP(
    ...
    **kwargs,
    ...
    return final_scale * (
        m
        @ Linear(
            d_embed, input_dim, **project_kwargs, zero_init=zero_init, tracker="mlp_in"
        )
        @ Flatten()
    )
...
def Attention(
    ...
    K = SplitIntoHeads(num_heads) @ Linear(
        num_heads * d_query, d_embed, **project_kwargs, tracker=f"k{layer_idx}"
    )
    V = SplitIntoHeads(num_heads) @ Linear(
        num_heads * d_value, d_embed, **project_kwargs, tracker=f"v{layer_idx}"
    )
    W = (
        Linear(
            d_embed,
...
def GPT(
    ...
    **kwargs,
    ...
    embed = Embed(
        ...
        vocab_size,
        dtype=dtype,
        max_inflation_factor=max_embed_inflation_factor,
        tracker="embed",
    )
    embed.tare()
    ...
    for i in range(num_blocks):
        ...
        att_block = (
            1 - residual_scale / (2 * num_blocks)
        ) * Identity() + residual_scale / (2 * num_blocks) * (
            att @ layernorm_substitute()
        )
        mlp_block = (
            1 - residual_scale / (2 * num_blocks)
        ) * Identity() + residual_scale / (2 * num_blocks) * (
            mlp @ layernorm_substitute()
        )
    ...
    if use_unembed:
        ...
        out = Linear(
            vocab_size, d_embed, **project_kwargs, zero_init=zero_init, tracker="out"
        )
    out.tare()
