...
class Atom(Module):
    ...
    def step(self, w, d_w, lr, g=None):
        ...
        dual_norm = (
            1 if g is None else jax.numpy.linalg.norm(g[0], "nuc")
        )  # jax.numpy.sum(d_w[0] * g[0])  # target norm messes this up
    ...
    def decay_step_project(
        ...
        w_projected = self.project(
            [w_stepped],
            w_max=w_max,
            wd=wd * max_update_norm,
...
class CompositeModule(Module):
    ...
    def dualize(self, grad_w, w=None, target_norm=1.0):
        if self.mass > 0:
            ...
            d_w1 = m1.dualize(
                grad_w1, w1, target_norm=target_norm * m1.mass / self.mass
            )
            d_w = d_w0 + d_w1
        else:
    ...
    def step(self, w, d_w, lr, g=None):
        ...
        g0, g1 = (
            g[: m0.atoms] if g is not None else None,
            g[m0.atoms :] if g is not None else None,
        )
    ...
    def dualize(self, grad_w, w=None, target_norm=1.0):
        if self.mass > 0:
            ...
            for m in self.children:
                ...
                d_w_m = m.dualize(
                    grad_w_m, w_m, target_norm=target_norm * m.mass / self.mass
                )
                d_w += d_w_m
